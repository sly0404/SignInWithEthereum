...
var currencies = Array.from(new Set(Object.values(currencies_1.cryptocurrenciesById)));
var duplicatedTickers = new Set();
currencies.reduce(function (acc, curr) {
...
currencies.forEach(function (c) {
    var _a;
    test("should find ".concat(c.name, " by id"), function () {
        expect((0, currencies_1.findCryptoCurrencyById)(c.id)).toEqual(c);
    });
    test("should find ".concat(c.name, " by ticker"), function () {
        try {
    ...
    test("should find ".concat(c.name, " by scheme"), function () {
        expect((0, currencies_1.findCryptoCurrencyByScheme)(c.scheme)).toEqual(c);
    });
    (_a = c.keywords) === null || _a === void 0 ? void 0 : _a.forEach(function (k) {
        test("should find ".concat(c.name, " with keyword ").concat(k), function () {
            expect((0, currencies_1.findCryptoCurrencyByKeyword)(k)).toEqual(c);
        });
    });
});
test("[LEGACY TEST] can find currency", function () {
    var bitcoinMatch = {
        id: "bitcoin",
        name: "Bitcoin"
    };
...
test("testnet currencies must also set disableCountervalue to true", function () {
    expect((0, currencies_1.listCryptoCurrencies)(true)
        .filter(function (c) { return c.isTestnetFor; })
        .filter(function (c) { return !c.disableCountervalue; })
        .map(function (c) { return c.id; })).toEqual([]);
});
...
test("Ethereum family convention: all ethereum testnet coins must derivate on the same cointype as the testnet it's for (e.g. ethereum ropsten is on 60)", function () {
    expect((0, currencies_1.listCryptoCurrencies)()
        .filter(function (e) {
        return e.family === "ethereum" && // ethereum family
            e.isTestnetFor && // is a testnet coin
            e.coinType !== (0, currencies_1.getCryptoCurrencyById)(e.isTestnetFor).coinType;
    } // it must use same coinType as the mainnet coin
    )
        .map(function (e) { return e.id; }) // to get a nice error if it fails
    ).toEqual([]);
});
